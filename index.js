...
function bufferToBase32(buffer) {
    ...
    while (numPadding > 0) {
        characterArray[characterArray.length - numPadding] = PADDING;
        numPadding--;
    }
...
// NB: currently, this will accept base32 strings without the character padding 
// as long as the characters in the string belong to the base32 alphabet
function base32ToBuffer(string) {

    if (typeof string !== 'string') {
        throw new Error('base32ToBuffer requires a String as input.');
    }

    // remove character padding, and convert to all lower case:
    string = string.replace(PADDING_REGEX, '').toLowerCase();

    const numBytes = Math.floor((string.length * 5) / 8);
...
function isValidBase32(string) {
    ...
    return BASE32_REGEX.test(string);
